...
typedef struct
{
  uint16_t MAIN_AXIS_NAMES, X2, Y2, Z2, Z3, Z4 REPEAT(E_STEPPERS, _EN_ITEM);
} per_stepper_uint16_t;
typedef struct
{
  uint32_t MAIN_AXIS_NAMES, X2, Y2, Z2, Z3, Z4 REPEAT(E_STEPPERS, _EN_ITEM);
} per_stepper_uint32_t;
typedef struct
{
  int16_t MAIN_AXIS_NAMES, X2, Y2, Z2, Z3, Z4;
} mot_stepper_int16_t;
typedef struct
{
  bool NUM_AXIS_LIST(X : 1, Y : 1, Z : 1, I : 1, J : 1, K : 1, U : 1, V : 1, W : 1), X2 : 1, Y2 : 1, Z2 : 1, Z3 : 1, Z4 : 1 REPEAT(E_STEPPERS, _EN1_ITEM);
} per_stepper_bool_t;
...
#define EEPROM_ASSERT(TST, ERR) \
  do                            \
  {                             \
    if (!(TST))                 \
    {                           \
      SERIAL_ERROR_MSG(ERR);    \
      eeprom_error = true;      \
    }                           \
  } while (0)
...
bool MarlinSettings::save()
{
  ...
  if (!EEPROM_START(EEPROM_OFFSET))
    return false;

  eeprom_error = false;

  // Write or Skip version. (Flash doesn't allow rewrite without erase.)
  TERN(FLASH_EEPROM_EMULATION, EEPROM_SKIP, EEPROM_WRITE)
  (ver);
...
#define _EN_ITEM(N) , .E##N = 30
    const per_stepper_uint32_t tmc_hybrid_threshold = {
        ...
        .X2 = 100, .Y2 = 100, .Z2 = 3, .Z3 = 3, .Z4 = 3 REPEAT(E_STEPPERS, _EN_ITEM)};
#undef _EN_ITEM
#endif
...
#else
    dummyf = 0;
    for (uint8_t q = _MAX(EXTRUDERS, 1); q--;)
      EEPROM_WRITE(dummyf);
#endif
  }
...
//
  ...
  //
  {
    ...
    LOOP_NUM_AXES(axis)
    backlash_distance_mm[axis] = backlash.get_distance_mm((AxisEnum)axis);
...
#define SET_STEPPING_MODE(ST)                                      \
  stepper##ST.stored.stealthChop_enabled = tmc_stealth_enabled.ST; \
  stepper##ST.refresh_stepping_mode();
  if (!validating)
  {
...
#if ENABLED(BACKLASH_GCODE)
  LOOP_NUM_AXES(axis)
  backlash.set_distance_mm((AxisEnum)axis, backlash_distance_mm[axis]);
...
#if ENABLED(DWIN_LCD_PROUI)
{
  ...
  if (!validating)
    DWIN_CopySettingsFrom(dwin_data);
}
#elif ENABLED(DWIN_CREALITY_LCD_JYERSUI)
{
  ...
  if (!validating)
    CrealityDWIN.Load_Settings(dwin_settings);
}
#endif
...
EEPROM_READ(ethernet_ip);
ethernet.ip = ethernet_ip;
EEPROM_READ(ethernet_dns);
ethernet.myDns = ethernet_dns;
EEPROM_READ(ethernet_gateway);
ethernet.gateway = ethernet_gateway;
EEPROM_READ(ethernet_subnet);
ethernet.subnet = ethernet_subnet;
#endif
...
if (!validating && TERN1(EEPROM_BOOT_SILENT, IsRunning()))
  report();
#endif
...
void MarlinSettings::store_mesh(const int8_t slot)
{
  ...
  if (status)
    SERIAL_ECHOLNPGM("?Unable to save mesh data.");
  else
    DEBUG_ECHOLNPGM("Mesh saved in slot ", slot);
...
#define DEFAULT_WJERK 0
  ...
  LOOP_NUM_AXES(axis)
  backlash.set_distance_mm((AxisEnum)axis, tmp[axis]);
#ifdef BACKLASH_SMOOTHING_MM
  backlash.set_smoothing_mm(BACKLASH_SMOOTHING_MM);
#endif
#endif
...
#if IS_KINEMATIC
  segments_per_second = (TERN_(DELTA, DELTA_SEGMENTS_PER_SECOND)
...
#define CONFIG_ECHO_MSG(V...) \
  do                          \
  {                           \
    CONFIG_ECHO_START();      \
    SERIAL_ECHOLNPGM(V);      \
  } while (0)
#define CONFIG_ECHO_MSG_P(V...) \
  do                            \
  {                             \
    CONFIG_ECHO_START();        \
    SERIAL_ECHOLNPGM_P(V);      \
  } while (0)
